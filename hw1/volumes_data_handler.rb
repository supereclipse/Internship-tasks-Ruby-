require './volumes_parser.rb'
require './extra_volume.rb'

class Volumes_Data_Handler
    @@volume_list = []
    
    # Возвращает список всех доп. жестких дисков для конкретного id ВM
    def self.get_all_volume_by_vm_id(vm_id)
        list = []
        Volumes_Parser.pull_volumes_hash.map {|row| list.append(row) if row[:Vm_id] == vm_id}
        list
    end    
  
    def self.get_volume_list
        @@volume_list
    end 

    # Создает массив обьектов класса ExtraVolume для каждого уникального id ВМ и соотв-го ему списка доп. жестких дисков
    def self.create_volume_list
        @@volume_list = []
        temp1 = []

        # Добавляю в список temp1 все id ВМ из спаршенного списка  
        Volumes_Parser.pull_volumes_hash.map {|x| temp1.append(x[:Vm_id])}

        # Для каждого уникального id ВМ создаю обьекст класса ExtraVolume
        temp1.uniq.each {|id| @@volume_list.append(ExtraVolume.new(id,self.get_all_volume_by_vm_id(id))) }
        @@volume_list          
    end

end